...
class GetCelebratedDealsData {
  ...
  private urlApi = 'https://www.gov.br/cgu/pt-br/assuntos/integridade-privada/acordo-leniencia/acordos-celebrados'
  ...
  async execute() {        
    ...
    const dataElements = await page.$$eval('table.mceItemTable tbody tr', async rows => {
      ...
      return rows.map(row => {
        ...
        return {
          ...
          attachments: Array.from(attachments).map(( attachment ) => attachment.getAttribute('href'))
        }
      });
    });
