...
class GetJudgedSanctioningProceedingsData {
  async execute({ initialDate = "01/01/2024", finalDate = "30/06/2024" }) {
    ...
    await page.goto(
      `https://conteudo.cvm.gov.br/sancionadores/index.html?lastNameShow=&lastName=&filtro=todos&dataInicio=${initialDateParsed}&dataFim=${finalDateParsed}&categoria0=%2Fsancionadores%2Fsancionador%2F&categoria1=%2Fsancionadores%2Fdespachos%2F&buscado=false&contCategoriasCheck=2&itensPagina=50`
    );
    ...
    for (const url of urlsList) {
      ...
      const value = await page.$eval(".contentTextoGeral", (property) => {
        ...
        const pdfUrl = property
          .querySelector(".contentVejaMais a")
          ?.getAttribute("href");

        const [, judgmentSessionDate] = items[0].innerText.split("\n");
...
export { GetJudgedSanctioningProceedingsData };
