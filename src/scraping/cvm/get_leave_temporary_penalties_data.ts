    ...
    const content = await page.$$eval("table.listing tr", (items) => {
      return items.map((item) => {
        ...
        if (participant.querySelector("p span span")?.textContent) {
          participantName =
            participant.querySelector("p span span")?.textContent;
        ...
        return {
          ...
          decisionType: decisionType.innerText,
          CVMjudgementDate: CVMjudgementDate.innerText,
          effectiveDateOfPenalty: effectiveDateOfPenalty.innerText,
