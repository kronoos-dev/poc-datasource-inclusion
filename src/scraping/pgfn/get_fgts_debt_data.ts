...
class GetFgtsDebtData {
  private async downloadFile(url: string) {
    try {      
      const pathMap = '/tmp/pgfn/fgts_debt'
  ...
  private async extractFile(url: string) {
    try {
      ...
      const extractDirectory = `assets/pgfn/fgts_debt/${directory}`
  ...
  async execute() {
    ...
    const listUrlAssets = await page.$$eval(`table`, table => {
      return Array.from(table).map(col => {
        return col.querySelectorAll('tr:nth-child(2) td a')[1].getAttribute('href') as string
      })
    });
